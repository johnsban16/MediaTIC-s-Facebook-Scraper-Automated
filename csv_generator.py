...
def addPostsAndCommentsToCSV(post, outfile_nodes, outfile_edges):
    ...
    if 'comments' in post:
        for comment in post['comments']['data']:
            ...
            if comment['id'] not in list_of_user_in_post:
                list_of_user_in_post.append(comment['id'])
                list_comment_id = [comment['id'], 'user' ,'', '',
                                    comment['message'], comment['created_time']]
                csv_data = []
                csv_data.insert(0, list_comment_id)
                save_csv(outfile_nodes, csv_data, file_mode="a")
                # insertar las aristas
                edge = [comment['id'], post['id']]
                csv_data = []
                csv_data.insert(0, edge)
...
def generateCSV(client_id, client_secret, mediaName, mediaId, sinceDate, untilDate):
    ...
    buildPostCSVs(client_id, client_secret, mediaId, since, until,
				  mediaName+'_nodes_posts_'+sinceFormated+'-'+untilFormated+'.csv',
				  mediaName+'_edges_posts_'+sinceFormated+'-'+untilFormated+'.csv',
				  version="2.10")

    #buildCommentsCSVs(client_id, client_secret, mediaId, since, until,
#					  mediaName+'_nodes_comments_'+sinceFormated+'-'+untilFormated+'.csv',
#					  mediaName+'_edges_comments_'+sinceFormated+'-'+untilFormated+'.csv',
#					  version="2.10")
    ...
    try:
        zipedData.write(mediaName+'_nodes_posts_'+sinceFormated+'-'+untilFormated+'.csv')
        zipedData.write(mediaName+'_edges_posts_'+sinceFormated+'-'+untilFormated+'.csv')
 #       zipedData.write(mediaName+'_nodes_comments_'+sinceFormated+'-'+untilFormated+'.csv')
 #       zipedData.write(mediaName+'_edges_comments_'+sinceFormated+'-'+untilFormated+'.csv')
    finally:
        zipedData.close()
        os.remove(mediaName+'_nodes_posts_'+sinceFormated+'-'+untilFormated+'.csv')
        os.remove(mediaName+'_edges_posts_'+sinceFormated+'-'+untilFormated+'.csv')
#       os.remove(mediaName+'_nodes_comments_'+sinceFormated+'-'+untilFormated+'.csv')
#       os.remove(mediaName+'_edges_comments_'+sinceFormated+'-'+untilFormated+'.csv')
    return fileN